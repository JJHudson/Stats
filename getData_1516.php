<?php
	...
	for($i=0; $i<count($teams); $i++) {
		...
		$oddsQuery = $mysqli->query('SELECT home_goals, away_goals, (CASE WHEN home_team = "'.$teams[$i].'" THEN away_team WHEN away_team = "'.$teams[$i].'" THEN home_team END) AS opponent, (CASE WHEN result = "H" && home_team = "'.$teams[$i].'" OR result = "A" && away_team = "'.$teams[$i].'" THEN "W" WHEN result = "A" && home_team = "'.$teams[$i].'" OR result = "H" && away_team = "'.$teams[$i].'" THEN "L" ELSE "D" END) as result, (CASE WHEN result = "H" && home_team = "'.$teams[$i].'" THEN home_odds WHEN result = "A" && away_team = "'.$teams[$i].'" THEN away_odds ELSE 0 END) as bet365 FROM results_1516 WHERE home_team = "'.$teams[$i].'" OR away_team = "'.$teams[$i].'"');

		while($row = $oddsQuery->fetch_assoc()){
			$temp_data = array('opponent'=>$row['opponent'], 'result'=>$row['result'],'home_goals'=>$row['home_goals'],'away_goals'=>$row['away_goals'], 'bet365'=>$row['bet365']);
